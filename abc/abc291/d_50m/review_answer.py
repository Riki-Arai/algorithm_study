...
for i in range(1, N):
    pre_i = i - 1
    for j in range(2):
        if card_lists[i][j] != card_lists[pre_i][0]:
            dp_lists[i][j] = (dp_lists[pre_i][0] + dp_lists[i][j])%998244353
    for j in range(2):
        if card_lists[i][j] != card_lists[pre_i][1]:
            dp_lists[i][j] = dp_lists[pre_i][1] + dp_lists[i][j]%998244353
